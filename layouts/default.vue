    ...
    <div id="TopMenu" class="w-full bg-[#fafafa] border-b md:block hidden">
      <ul class="flex items-center justify-end text-xs text-[#333333] font-light px-2 h-10 bg-[#fafafa] max-w-[1200px]">
        ...
        <li @mouseenter="isAccountMenu = true" @mouseleave="isAccountMenu = false"
          class="relative flex items-center px-2.5 h-full hover:text-[#ff4646] cursor-pointer" :class="isAccountMenu
            ? 'bg-white z-40 shadow-[0_15px_100px_40px_rgba(0,0,0,0.3)]'
            : 'border border-[#fafafa]'
            ">
          <Icon name="ph:user-thin" size="17" class="mr-1" />
          Account
      ...
      <div class="flex lg:justify-start justify-between gap-10 max-w-[1150px] w-full px-3 py-5 mx-auto">
        ...
        <div class="max-w-[700px] w-full md:block hidden">
          <div class="flex items-center border-2 border-[#ff4646] rounded-md w-full">
            <input class="w-full placeholder-gray-400 text-sm pl-3 focus:outline-none" v-model="searchItem"
              placeholder="kitchen accessories" type="text" />
            <Icon v-if="isSearch" name="eos-icons:loading" size="25" class="mr-2" />
            ...
            <div v-if="false" class="p-1">
              <NuxtLink to="`/item/1" class="flex items-center justify-between w-full cursor-pointer hover:bg-gray-100">
                <div class="flex items-center">
                  <img src="https://picsum.photos/id/82/300/320" class="rounded-sm" width="40">
                  <div class="truncate ml-2">test</div>
                </div>
                <div class="truncate">$ 98.98</div>
              </NuxtLink>
            </div>
          </div>
        </div>
